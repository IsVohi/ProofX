...
function verifyCircuitArtifacts() {
    ...
    if (!wasmExists || !zkeyExists) {
        console.error('⚠️  WARNING: ZK circuit artifacts not found!');
        console.error('   WASM:', wasmExists ? '✅' : '❌', CIRCUIT_WASM_PATH);
        console.error('   ZKEY:', zkeyExists ? '✅' : '❌', CIRCUIT_ZKEY_PATH);
...
app.post('/prove', async (req, res) => {
    try {
        ...
        if (useRealZK && input.assets && input.liabilities && input.threshold) {
            ...
            // ═══════════════════════════════════════════════════════════════
            console.log("⚠️  Using SIMULATED proof (circuit not ready or legacy input format)");
...
app.post('/verify-authorization', (req, res) => {
    try {
        ...
        console.log("🔐 Verifying Keychain authorization...");
        ...
        console.log("✅ Keychain authorization verified:", verification.recoveredAddress);
...
╚════════════════════════════════════════════════════════════════╝
  ...
  🔐 ZK Status: ${artifactsReady ? '✅ REAL PROOFS ENABLED' : '⚠️  SIMULATED MODE'}
  🛡️  Authorization: Keychain (EIP-191)
    `);
});
